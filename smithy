...
bootstrap_virtualenv()
{
    ...
    local deps=$(cat requirements.txt test-requirements.txt  | grep -v '^$\|^\s*\#')
    dump_list $deps
    echo "Please wait..."
    if [ "$VERBOSE" == "0" ]; then
        $pip install -r requirements.txt -r test-requirements.txt > /dev/null 2>&1
    else
        $pip install -v -r requirements.txt -r test-requirements.txt
    fi
    if [ "$?" != "0" ]; then
        return 1
    fi
...
run_smithy()
{
    # Runs anvil using the python binary in the virtualenv which will have
    # linkage to the anvil specific dependencies.
