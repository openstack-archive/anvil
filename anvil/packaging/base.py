...
class DependencyHandler(object):
    ...
    def _examine_download_dir(self, reqs_downloaded, pip_download_dir):
        pip_names = set([p.key for p in reqs_downloaded])
        ...
        for filename in what_downloaded:
            ...
            if req.key not in pip_names:
                LOG.info("Dependency %s was automatically included.",
                         colorizer.quote(req))
        return (reqs_downloaded, what_downloaded)
    ...
    def download_dependencies(self):
        ...
        reqs_to_download = pip_helper.sort_requirements((pip_helper.extract_requirement(p)
        ...
        if (sh.isfile(self.downloaded_flag_file) and
            ...
            def try_download(attempt):
                ...
                header = header % (len(reqs_to_download), attempt)
                utils.log_iterable(reqs_to_download, logger=LOG, header=header)
                ...
                pip_helper.download_dependencies(self.download_dir, pips_to_download,
                                                 output_filename)
            utils.retry(self.MAX_PIP_DOWNLOAD_ATTEMPTS, self.PIP_DOWNLOAD_DELAY, try_download)
        ...
        return self._examine_download_dir(reqs_to_download, self.download_dir)
