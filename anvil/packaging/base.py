...
class DependencyHandler(object):
    ...
    def _clean_pip_requires(self, requires_files):
        ...
        def get_replacement(filename, source_req, forced_by_key):
            source_name = source_req.key
            try:
                return forced_by_key[source_name]
            except KeyError:
                pass
            install_egg = None
            for egg_info in self._python_eggs:
                if egg_info['name'] == source_name:
                    install_egg = egg_info
                    break
            if not install_egg:
                return None
            # Ensure what we are about to install will actually work with the
            # desired version (or abort).
            install_req = pkg_resources.Requirement.parse("%s<=%s"
                                                          % (install_egg['name'],
                                                             install_egg['version']))
            if source_req not in install_req:
                msg = ("Can not replace '%s' with '%s', incompatible"
                       " requirement found in %s")
                raise exc.DependencyException(msg % (source_req,
                                                     install_egg['req'],
                                                     filename))
            return install_egg['req']
        ...
        for fn in requires_files:
            ...
            for i, line in enumerate(old_lines):
                try:
                    ...
                    replace_req = get_replacement(fn, source_req, forced_by_key)
                    if replace_req is not None:
