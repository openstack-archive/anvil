...
class DependencyHandler(object):
    ...
    def __init__(self, distro, root_dir, instances, opts):
        ...
        self.forced_distro_pips = []
        forced_distro_pips = self.distro.get_dependency_config("forced_pips", quiet=True)
        if forced_distro_pips:
            for line in forced_distro_pips:
                self.forced_distro_pips.append(pip_helper.extract_requirement(line))
        self.forced_distro_pips = sorted(self.forced_distro_pips, cmp=sort_req)
    ...
    def _scan_pip_requires(self, requires_files):
        ...
        for pkg in self.forced_distro_pips:
            forced_by_key[pkg.key] = pkg
        mutations = 0
