...
class YumDependencyHandler(base.DependencyHandler):
    ...
    def prepare(self):
        super(YumDependencyHandler, self).prepare()
    ...
    def _write_all_deps_package(self):
        spec_filename = sh.joinpths(
            self.deps_dir, "%s.spec" % self.OPENSTACK_DEPS_PACKAGE_NAME)
        cmdline = [self.py2rpm_executable, "--convert"] + self.pips_to_install
        output = sh.execute(*cmdline)

        sh.deldir(self.rpmbuild_dir)
        sh.mkdir(self.deps_repo_dir, recurse=True)
        for filename in sh.listdir(self.deps_repo_dir):
            if sh.basename(filename).startswith(
                    self.OPENSTACK_DEPS_PACKAGE_NAME):
                sh.unlink(filename)
...
""" % (self.OPENSTACK_DEPS_PACKAGE_NAME, today.year, today.month, today.day)
    ...
    def _build_dependencies(self):
        ...
        out_filename = sh.joinpths(self.deps_dir, "py2rpm.out")
        utils.log_iterable(package_files, logger=LOG,
    ...
    def _create_deps_repo(self):
        for filename in sh.listdir(sh.joinpths(self.rpmbuild_dir, "RPMS"),
            ...
            new_name = sh.joinpths(self.deps_repo_dir, sh.basename(filename))
            if sh.isfile(new_name):
                sh.unlink(new_name)
            sh.move(filename, new_name)
        cmdline = ["createrepo", self.deps_repo_dir]
        LOG.info("Creating repo at %s" % self.deps_repo_dir)
        sh.execute(*cmdline)
        LOG.info("Writing anvil.repo to %s" % self.anvil_repo_filename)
        (_fn, content) = utils.load_template('packaging', 'anvil.repo')
        params = {"baseurl": "file://%s" % self.deps_repo_dir}
        sh.write_file(
