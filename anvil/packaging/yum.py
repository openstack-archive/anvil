...
class YumDependencyHandler(base.DependencyHandler):
    ...
    def _write_spec_file(self, pkg_dir, rpm_name, version, template_name, params):
    ...
    def _build_openstack_package_client(self, pkg_dir):
        ...
        version = self._python_setup_py_get(pkg_dir, "version")
        ...
        spec_filename = self._write_spec_file(
            pkg_dir, name, version, "python-commonclient.spec", params)
    ...
    def _build_openstack_package_server(self, pkg_dir):
        ...
        for line in spec_content.splitlines():
            ...
            if line.startswith("Source") and line.endswith(".init"):
                try:
                    ...
                    if "-" not in bin_name:
                        # server has `-all` suffix
                        bin_name = "%s-all" % bin_name
                    init_params["bin"] = bin_name
                    sh.write_file(
        ...
        spec_filename = self._write_spec_file(
            pkg_dir, "openstack-%s" % name, version, spec_filename, {})
        self._build_from_spec(pkg_dir, spec_filename)
    ...
    def _build_openstack_package(self, pkg_dir):
        ...
        if sh.isfile(spec_filename):
            spec_filename = self._write_spec_file(
                pkg_dir, component_name, 1, "%s.spec" % component_name, {})
            self._build_from_spec(pkg_dir, spec_filename)
            return
        ...
        for inst in self.instances:
            try:
                for pack in inst.packages:
                    rpm_names.add(pack["name"])
            except (AttributeError, KeyError):
                pass
            daemon_to_package = self.distro._components[
                inst.name].get("daemon_to_package", {})
            for key in inst.subsystems.iterkeys():
                try:
                    rpm_names.add(daemon_to_package[key])
                except KeyError:
                    rpm_names.add("openstack-%s-%s" % (inst.name, key))
