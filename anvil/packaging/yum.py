...
class YumDependencyHandler(base.DependencyHandler):
    ...
    def _filter_download_requires(self):
        ...
        if satisfied_list:
            ...
            for (req, rpm_name, rpm_info) in satisfied_list:
                ...
                yum_provided.write("%s, %s\n" % (req, json.dumps(rpm_info)))
            for r in sorted(repos.keys()):
                ...
                utils.log_iterable(sorted(repos[r]), logger=LOG, header=header,
                                   color=None)
            sh.write_file(self.yum_satisfies_filename, yum_provided.getvalue())
        else:
            sh.write_file(self.yum_satisfies_filename, "")
    ...
    def _build_dependencies(self):
        ...
        if yum_provided:
            more_yum_satisfies = six.StringIO()
            for (req, rpm_info) in yum_provided:
                more_yum_satisfies.write("%s, %s\n" % (req, json.dumps(rpm_info)))
            sh.append_file(self.yum_satisfies_filename, more_yum_satisfies.getvalue())
