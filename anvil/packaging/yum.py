...
class YumDependencyHandler(base.DependencyHandler):
    ...
    def _my_package_deps(self):
        if self._my_deps is not None:
            return self._my_deps
        my_deps = []
        ...
        for fn in ('requirements.txt', 'test-requirements.txt',):
            try:
                reqs.extend(pip_helper.parse_requirements(sh.load_file(fn)))
            except IOError:
                pass
        my_deps.extend(self._convert_names_python2rpm([p.key for p in reqs]))
        self._my_deps = set(my_deps)
        return self._my_deps

    def uninstall(self):
        super(YumDependencyHandler, self).uninstall()
        if self.tracereader.exists():
            no_removes = self._my_package_deps()
            remove_pkgs = []
            for p in self.tracereader.packages_installed():
                if p in no_removes:
                    LOG.debug("Not removing %s even though it was recorded as being"
                              " installed since it is an anvil runtime dependency.", p)
                else:
