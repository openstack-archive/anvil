...
class YumDependencyHandler(base.DependencyHandler):
    ...
    def build_binary(self):
        ...
        def pre_build():
            ...
            build_requires = ''
            try:
                build_requires = sh.load_file(self.rpm_build_requires_filename)
            except IOError as e:
                if e.errno != errno.ENOENT:
                    raise
            build_requires = set(pkg_resources.yield_lines(build_requires))
            ...
            for repo_name in self.REPOS:
                ...
                for path in list_src_rpms(repo_dir):
                    ...
                    if package_name in build_requires:
                        prebuilds.append(path)
                        build_requires.discard(package_name)
                if prebuilds:
