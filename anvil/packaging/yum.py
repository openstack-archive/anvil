...
class YumDependencyHandler(base.DependencyHandler):
    ...
    def build_binary(self):
        ...
        def pre_build():
            ...
            rpm_build_requires = ''
            try:
                rpm_build_requires = sh.load_file(self.rpm_build_requires_filename)
            except IOError as e:
                if e.errno != errno.ENOENT:
                    raise
            rpm_build_requires = set([s.strip()
                                      for s in rpm_build_requires.split("\n")
                                      if s.strip()])
            if not rpm_build_requires:
                return
            for repo_name in self.REPOS:
                ...
                for path in list_src_rpms(repo_dir):
                    ...
                    if package_name in rpm_build_requires:
                        prebuilds.append(path)
                        rpm_build_requires.discard(package_name)
                if prebuilds:
    ...
    def _build_from_spec(self, instance, spec_filename, patches=None):
        ...
        rpm_requires = spec_details.get('requires', [])
        if rpm_requires:
