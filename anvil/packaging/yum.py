    ...
    def _build_dependencies(self):
        ...
        def filter_package_files(package_files):
            ...
            for filename in package_files:
                ...
                p_reqs.append(package_details['req'])
                p_keys.append(package_details['req'].key)
            p_rpm_names = self._convert_names_python2rpm(p_keys)
            filtered_files = []
            for (filename, req, rpm_name) in zip(package_files, p_reqs,
                                                 p_rpm_names):
