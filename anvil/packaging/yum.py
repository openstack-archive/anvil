...
class YumDependencyHandler(base.DependencyHandler):
    ...
    def _package_parameters(self, instance):
        ...
        # NOTE(imelnikov): templates expect the following parameter to be 0 or 1
        if instance.get_option("package_tests", default_value=True):
            params["no_tests"] = 0
        else:
            params["no_tests"] = 1
        return params
        ...
        if sh.isfile(requires_filename):
            ...
            if requires_python:
                result = self._convert_names_python2rpm(
                    [r.key for r in requires_python])
        return result
