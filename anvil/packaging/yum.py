...
class YumDependencyHandler(base.DependencyHandler):
    ...
    def _find_template_and_rpm_name(self, instance):
        ...
        search_names = [
            (instance_name, "%s.spec" % instance_name),
        ]
        try:
            ...
            search_names.append(("openstack-%s" % (egg_name),
                                 "openstack-%s.spec" % (egg_name)))
            search_names.append((egg_name, "%s.spec" % (egg_name)))
    ...
    def _build_openstack_package(self, instance):
        ...
        (rpm_name, template_name) = self._find_template_and_rpm_name(instance)
        try:
            egg_name = instance.egg_info['name']
