...
def group_builds(components, ensure_component='general'):
    if not components:
        return []
    ...
    for c in components:
        if isinstance(c, six.string_types):
            ...
            project_name = list(c)[0]
            stage_id = int(c[project_name])
    ...
    for i in sorted(six.iterkeys(stages)):
        ...
        if ensure_component and ensure_component not in stage:
            stage.insert(0, ensure_component)
