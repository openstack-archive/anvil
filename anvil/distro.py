...
class Distro(object):
    def __init__(self,
        ...
        self._dependency_handler = self._inject_platform(dependency_handler, kwargs)
        self._commands = commands
        self._components = components

    @staticmethod
    def _inject_platform(merged_into, potential_data):
        if 'platform_overrides' in potential_data:
            overrides = potential_data['platform_overrides']
            d = _linux_distribution()
            if d in overrides:
                LOG.info("Merging in 'platform_overrides' that matched distro %s", d)
                return utils.recursive_merge(merged_into, overrides[d])
            else:
                LOG.debug("Distro %s not in 'platform_overrides' (valid are %s)",
                          d, list(six.iterkeys(overrides)))
        return merged_into
    ...
    def merge(self, **kwargs):
        self._dependency_handler = self._inject_platform(self._dependency_handler,
                                                         kwargs)
