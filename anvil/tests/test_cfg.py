...
class TestYamlRefLoader(unittest.TestCase):
    ...
    def test_load__complex(self):
        """Test parsing of the complex yaml without references."""
        self.sample = """
        # some comments...
    ...
    def test_load__simple_ref(self):
    ...
    def test_load_associated_ref(self):
        self.sample = """
        stable: 9
        ...
        self.assertEqual(processed['sample2']['stable'], 10)

    def test_load__complex_ref(self):
        """Test loading references in many different ways"""
        self.sample = """
        stable: 9
    ...
    def test_load__complex_path_ref(self):
        self.sample = """
        l-numbers:
          - 1
          - 2
          - 3
          - "$(sample2:d-numbers:four)"
          - "$(sample2:d-numbers:five)"

        d-numbers:
          one: "$(sample2:l-numbers:0)"
          two: "$(sample2:l-numbers:1)"
          three: "$(sample2:l-numbers:2)"
          four: "$(sample2:d-numbers:four)"
          five: "$(sample2:d-numbers:five)"
        ...
        yet-one: "$(sample:d-numbers:one)"
        1-plus-2: "1 + $(sample:d-numbers:two)"
        2-plus-1: "$(sample:d-numbers:two) + 1"
        2-plus-2: "$(sample:d-numbers:two) + $(sample:l-numbers:1)"
        """
        ...
        self.assertEqual(processed['l-numbers'], [1, 2, 3, 4, 5])
        self.assertEqual(
            processed['d-numbers'],
            utils.OrderedDict([('five', 5), ('four', 4), ('one', 1),
                               ('three', 3), ('two', 2)])
        )
        self.assertEqual(
            processed['n-numbers'],
            utils.OrderedDict([(1, 1), (2, 2), (3, 3)])
        )
        self.assertEqual(processed['yet-one'], 1)
        self.assertEqual(processed['1-plus-2'], "1 + 2")
        self.assertEqual(processed['2-plus-1'], "2 + 1")
        self.assertEqual(processed['2-plus-2'], "2 + 2")
