...
class Action(object):
    ...
    def _construct_instances(self, persona):
        ...
        groupings = []
        for component_group in persona.wanted_components:
            ...
            for c in component_group:
                if c in components_created:
                    raise RuntimeError("Can not duplicate component %s in a"
                                       " later group %s" % (c, component_group.group))
            ...
            groupings.append((component_group.group, instances))
        return groupings
    ...
    def _run_phase(self, functors, instances, phase_name, *inv_phase_names):
    ...
    def run(self, persona):
        ...
        for group in persona.wanted_components:
            utils.log_iterable(group,
                               header="Activating group %s in the following order" % colorizer.quote(group.group),
                               logger=LOG)
