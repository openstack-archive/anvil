...
class Action(object):
    ...
    def _merge_subsystems(self, distro_subsystems, desired_subsystems):
        ...
        for subsystem_name in desired_subsystems:
            ...
            subsystems[subsystem_name] = copy.deepcopy(
                distro_subsystems.get(subsystem_name, {}))
        return subsystems

    def _construct_siblings(
            self, name, siblings, base_params, sibling_instances):
        ...
        for (action, entry_point) in siblings.items():
            sibling_params = utils.merge_dicts(
                base_params,
                self.cli_opts,
                preserve=True)
            ...
            a_sibling = importer.construct_entry_point(
                entry_point, **sibling_params)
            # Update the sibling we are returning and the corresponding
            # siblings for that action (so that the sibling can have the
            # correct 'sibling' instances associated with it, if it needs
            # those...)
    ...
    def _construct_instances(self, persona):
        ...
        for c in wanted_components:
            ...
            # if this is not useful, we can give them full access, unsure if
            # its worse or better...
            ...
            sibling_params['options'] = self._merge_options(c,
                                                            component_opts=self._get_interpolated_options(
                                                            c),
                                                            distro_opts=d_component.options,
                                                            persona_opts={})
            LOG.debug(
                "Constructing %r %s siblings...",
                c,
                len(d_component.siblings))
            my_siblings = self._construct_siblings(
                c,
                d_component.siblings,
                sibling_params,
                sibling_instances)
            ...
            instance_params = utils.merge_dicts(
                instance_params,
                self.cli_opts,
                preserve=True)
            instances[
                c] = importer.construct_entry_point(
                    d_component.entry_point,
                    **instance_params)
        return instances
    ...
    def _get_phase_filename(self, phase_name):
        # Do some canonicalization of the phase name so its in a semi-standard
        # format...
    ...
    def _run_phase(self, functors, component_order,
                   instances, phase_name, *inv_phase_names):
        """
        ...
        if not phase_name:
            ...
            phase_recorder = phase.PhaseRecorder(
                self._get_phase_filename(phase_name))
        ...
        if inv_phase_names:
            for n in inv_phase_names:
                if not n:
                    ...
                    neg_phase_recs.append(
                        phase.PhaseRecorder(
                            self._get_phase_filename(
                                n)))
        ...
        # Run all components which have not been ran previously (due to phase
        # tracking)
        for c in component_order:
            result = None
            instance = instances[c]
    ...
    def run(self, persona):
        ...
        LOG.info(
            "Processing components for action %s.",
