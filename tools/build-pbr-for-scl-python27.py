...
import exceptions
import fnmatch
import importlib  # required by spec2scl
import logging
import os
import shutil
import subprocess
...
def install_dependencies():
    ...
    subprocess.check_call(args=['sudo', 'pip', 'install', 'importlib'])
...
def main(**kwargs):
    ...
    with contextlib.closing(tarfile.open(name=pbr_tgz_fn,
        ...
        subprocess.check_call(args=[kwargs['python'], setup_fn,
                              'bdist_rpm'])
        ...
        new_build_tgz_dirn = 'python27-{0}'.format(old_build_tgz_dirn)
        ...
        # Copy the rpm
        rpm_matches = walk_and_fnmatch(build_dirn, 'python27-pbr-*.noarch.rpm')
